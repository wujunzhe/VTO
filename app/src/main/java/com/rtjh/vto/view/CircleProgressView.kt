...
import com.rtjh.vto.utils.TypeUtils.Companion.isNum
...
class CircleProgressView(context : Context?, attrs : AttributeSet?) : View(context, attrs) {
    ...
    private fun drawIcon(canvas : Canvas) {
        mValue?.let { it ->
            ...
            if (intValue != null) {
                // 计算主值文本的基线
                val textBaseline = (centerPosition.y + mValuePaint.textSize / 4f) + 10

                // 计算主值文本的X坐标，使其向左移动
                val valueTextX = centerPosition.x.toFloat() - mValuePaint.measureText(intValue.toString()) * 0.1f
                // 绘制主值文本
                ...
                mUnit?.let {
                    // 设置单位文本的字体大小
                    val unitTextSize = mValuePaint.textSize * 0.4f
                    mValuePaint.textSize = unitTextSize

                    // 计算单位文本的位置
                    val unitTextBaseline = textBaseline - mValuePaint.descent() - unitTextSize
                    val unitTextX = valueTextX + mValuePaint.measureText(intValue.toString()) * 2f
    ...
    private fun setupGradientCircle() {
        if (mGradientColors != null && mGradientColors!!.size >= 2) {
            // 计算渐变中每个颜色在渐变过程中的位置
            val colorPositions = FloatArray(mGradientColors!!.size)

            // 计算等差数列，确保 endColor 在最后
            val step = 1.0f / (mGradientColors!!.size - 1)
            for (i in mGradientColors!!.indices) {
                colorPositions[i] = i * step
            }
            // 最后一个颜色的位置设为 1，以确保达到 mValue 时显示终止颜色
            colorPositions[mGradientColors!!.size - 1] = 1f

            // 创建具有多个颜色的扫描渐变
            mSweepGradient = SweepGradient(
                centerPosition.x.toFloat(), centerPosition.y.toFloat(),
                mGradientColors!!, colorPositions
            )

            // 设置渐变的起始和结束位置
            val matrix = Matrix()
            val adjustedStartAngle = mStartAngle - 9f
            matrix.setRotate(adjustedStartAngle, centerPosition.x.toFloat(), centerPosition.y.toFloat())
